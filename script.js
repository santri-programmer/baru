...
function setupEventListeners() {
  ...
  cachedElements.kategoriDonatur.addEventListener("change", async function () {
    ...
    // Gunakan batched updates untuk performa lebih baik
    batchedUpdates.add(async () => {
      // Tunggu sampai data hari ini selesai dimuat
      await loadDataHariIni(kategori);

      // Baru muat dropdown setelah data terinput sudah diketahui
  ...
  // Quick amount buttons dengan tambahan "Tidak Mengisi"
...
async function checkOfflineData() {
  try {
    ...
    if (queue.length > 0) {
      showNotification(
        `📦 Ada ${queue.length} data offline yang tersimpan`,
        true
      );
...
async function handleUploadSuccess(kategori, message = "") {
  ...
  showUploadStatus(
    `✅ ${
      message ||
      `Data berhasil diupload untuk kategori ${kategoriLabel[kategori]}`
    }`,
    true
  );
...
function renderTabelTerurut(kategori) {
  ...
  sortedData.forEach((item) => {
    ...
    editBtn.addEventListener("click", () =>
      editRow(row, kategori, item.donatur, item.id)
    );
    ...
    deleteBtn.addEventListener("click", () =>
      hapusRow(kategori, item.donatur, item.id)
    );
...
function editRow(row, kategori, donaturLama, itemId) {
  ...
  saveBtn.addEventListener("click", () =>
    saveRow(row, kategori, donaturLama, itemId)
  );
